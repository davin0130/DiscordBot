...
async def on_message(message):
    ...
    if user.id != client.user.id:
        ...
        # 관리 기능
        if Admin.adminList.adminList(user) == True:
            Admin.adminList.searchComment(content)
...
async def on_voice_state_update(member, before, after):
    ...
    if member.id != client.user.id:
        if after.channel == None:
            DAO.Voice.updateVoice(current_time, member.id, before.channel.id)
        else:
            if len(DAO.Voice.selectIDVoice(member.id, after.channel.id)) == 0:
                DAO.Voice.insertVoice(member.id, after.channel.id, current_time)
            else:
                DAO.Voice.resetVoice(current_time, member.id, after.channel.id)
